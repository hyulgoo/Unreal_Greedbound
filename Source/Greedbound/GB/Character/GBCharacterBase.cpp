    ...
    : Super(ObjectInitializer)
{
    ...
    Combat = CreateDefaultSubobject<UGBCombatComponent>(TEXT("CombatComponent"));
    MovementStateComponent = CreateDefaultSubobject<UGBMovementStateComponent>(TEXT("MovementStateComponent"));
    static ConstructorHelpers::FClassFinder<UGameplayEffect> WeaponEquipGameplayEffectRef(TEXT("/Game/GB/GAS/Effect/GE_Instant_WeaponEquip.GE_Instant_WeaponEquip_C"));
    if (WeaponEquipGameplayEffectRef.Class)
    {
        WeaponEquipGameplayEffectClass = WeaponEquipGameplayEffectRef.Class;
    }
}
...
{
    Combat->SetCombatState(NewState);
}
...
void AGBCharacterBase::EquipWeapon(AGBWeapon* NewWeapon)
{
    ...
    GB_NULL_CHECK(AbilitySystemComponent);
    GB_NULL_CHECK(WeaponEquipGameplayEffectClass);
...
void AGBCharacterBase::OnAttackEventRecieved(const FGameplayEventData* Payload)
{
    ...
    if (AbilitySystemComponent->HasMatchingGameplayTag(GBTag::State_EquipState_Equipped))
    {
        ...
        {
            Combat->SetWeaponTrailEffect(true);
        }
        else if (Payload->EventTag == GBTag::Event_Common_TrailAttackEnd)
        {
            Combat->SetWeaponTrailEffect(false);
        }
    }
}
